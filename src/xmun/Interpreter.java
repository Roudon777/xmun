...
//> Functions interpreter-constructor
  Interpreter() {
    ...
    globals.define("List",new Callable()  {
    	@Override
        public int arity() { return 1; }
    	 @Override
         public Object call(Interpreter interpreter,
                 List<Object> arguments) {
    		 int a = (int)(double)arguments.get(0);
    		 return new xmunList(a);
         } 
    } );
...
//> Inheritance interpret-superclass
    ...
    if (stmt.superclass != null) {
      ...
      if (!(superclass instanceof XClass)) {
...
//> Inheritance interpreter-construct-class
    XClass klass = new XClass(stmt.name.lexeme,
        (XClass)superclass, methods);
...
//> Classes construct-function
    XFunction function = new XFunction(stmt, environment,
                                           false);
//< Classes construct-function
...
//> Classes interpreter-visit-get
  ...
  public Object visitGetExpr(Expr.Get expr) {
    ...
    if (object instanceof XInstance) {
      return ((XInstance) object).get(expr.name);
    }
...
//> Classes interpreter-visit-set
  ...
  public Object visitSetExpr(Expr.Set expr) {
    ...
    if (!(object instanceof XInstance)) { // [order]
      throw new RuntimeError(expr.name,
                             "Only instances have fields.");
    }

    Object value = evaluate(expr.value);
    ((XInstance)object).set(expr.name, value);
    return value;
  }
//< Classes interpreter-visit-set
//> Inheritance interpreter-visit-super
  ...
  public Object visitSuperExpr(Expr.Super expr) {
    ...
    XClass superclass = (XClass)environment.getAt(
        distance, "super");
//> super-find-this

    XInstance object = (XInstance)environment.getAt(
        distance - 1, "this");
//< super-find-this
//> super-find-method

    XFunction method = superclass.findMethod(expr.method.lexeme);
//> super-no-method
